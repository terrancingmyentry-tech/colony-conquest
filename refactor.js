...
// Step 1: Create the module-level isValidStarterPos function
const moduleFunc = `// Validate starter position (used by both socket handler and AI)
...
// Step 2: Insert module-level function before '// -------------------------\n// AI behavior'
content = content.replace(
  /\/\/ -+\n\/\/ AI behavior/,
  moduleFunc + '// -------------------------\n// AI behavior'
);

// Step 3: Remove local function from inside runAI
const localFuncPattern = /  function isValidStarterPos\(x,y\) \{[\s\S]*?return \{ ok: true \};\s*\}/;
content = content.replace(localFuncPattern, '');

// Step 4: Update AI call to pass 'st' parameter
content = content.replace(
  /const validation = isValidStarterPos\(p\.x, p\.y\);/,
