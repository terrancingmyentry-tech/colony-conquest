// public/index.js
// Lobby UI + create/join logic. Exposes socket on window for game-client to reuse.
...
function populateHostSelects() {
  ...
  aiCountSel.innerHTML=''; for (let i = 0; i <= 8; i++) aiCountSel.appendChild(new Option(i,i));
...
createBtn.onclick = () => {
  ...
  const cfg = {
    ...
    aiCount: Number(aiCountSel.value),
...
joinBtn.onclick = () => {
  ...
  socket.emit('joinRoom', { roomId: code, name, color }, (res) => {
    ...
    renderPlayersList(res.players || []);
    leaveBtn.style.display = 'inline-block';
    startBtn.style.display = (res.players && res.players[0] && res.players[0].sid === socket.id) ? 'inline-block' : 'none';
  });
};
...
startBtn.onclick = () => {
  ...
  socket.emit('startGame', { roomId: localRoom }, (res) => {
...
leaveBtn.onclick = () => {
  if (!localRoom) return;
  socket.emit('leaveRoom', { roomId: localRoom }, (res) => {
    if (res && res.ok) {
...
socket.on('lobbyUpdate', ({ players, config }) => {
  ...
  if (players && players.length && socket.id === players[0]?.sid) startBtn.style.display = 'inline-block';
});
